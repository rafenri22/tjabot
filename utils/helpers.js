...
class BotHelpers {
    ...
    static isValidCommand(text) {
      const commands = ['/TJA-', '/jomox', '/JOMOX', '/siapa', '/verifikasi', '/VERIFIKASI', '/info', '/kembaran', '/KEMBARAN'];
    ...
    static getRandomOldIndonesianName() {
      const names = [
        'Pak Budi', 'Bang Anto', 'Om Slamet', 'Pak Agus', 'Bang Rudi', 'Om Bambang', 'Pak Dedi',
        'Bang Harto', 'Om Sutrisno', 'Pak Wawan', 'Bang Iwan', 'Om Tarno', 'Pak Joko', 'Bang Endro',
        'Om Sukimin', 'Pak Dadang', 'Bang Eko', 'Om Purwanto', 'Pak Hendra', 'Bang Yanto',
        'Om Kusno', 'Pak Supri', 'Bang Gunawan', 'Om Teguh', 'Pak Wahyu', 'Bang Indra',
        'Om Sugeng', 'Pak Rohmat', 'Bang Danu', 'Om Winarto', 'Pak Gito', 'Bang Purnomo',
        'Om Suryono', 'Pak Tri', 'Bang Handoko', 'Om Marno', 'Pak Sarno', 'Bang Mukidi',
        'Om Karno', 'Pak Sugiarto', 'Bang Hermanto', 'Om Widodo', 'Pak Sumadi', 'Bang Ridwan',
        'Om Suharto', 'Pak Yusuf', 'Bang Syahrul', 'Om Mulyo', 'Pak Rizki Lama', 'Bang Wahidin',
        'Om Supeno', 'Pak Darno', 'Bang Subur', 'Om Kartono', 'Pak Sundoro', 'Bang Surono',
        'Om Prijono', 'Pak Sutejo', 'Bang Dwi Tua', 'Om Harjono', 'Pak Slamet Tua', 'Bang Markum',
        'Om Sumanto', 'Pak Sugito', 'Bang Harsono', 'Om Paijo', 'Pak Kusman', 'Bang Suparman',
        'Om Sukirno', 'Pak Sugimin', 'Bang Suryadi', 'Om Parto', 'Pak Supriyanto', 'Bang Sutikno',
        'Om Sukardi', 'Pak Sunaryo', 'Bang Roni Tua', 'Om Martono', 'Pak Subagyo', 'Bang Wasono',
        'Om Parmin', 'Pak Sukino', 'Bang Setyo', 'Om Pardjo', 'Pak Sunarto', 'Bang Mulyono',
        'Om Sastro', 'Pak Sugiyanto', 'Bang Suratman', 'Om Parjono', 'Pak Mulyadi Tua', 'Bang Suyono',
        'Om Sukamto', 'Pak Suryanto', 'Bang Sugianto', 'Om Pariman', 'Pak Sungguh', 'Bang Suhendra Tua',
        'Om Subardi', 'Pak Suparto', 'Bang Sukamdi', 'Om Pardi', 'Pak Sukatno', 'Bang Suroyo',
        'Om Sutarjo', 'Pak Suradi', 'Bang Suparno', 'Om Pariyo'
      ];
    ...
    static getRandomJomoxResponse(name) {
      const responses = [
        ...
        `${Math.floor(Math.random() * 100)}% ${name} punya gen jomok turunan keluarga`,
        `${name} tingkat jomoknya ${Math.floor(Math.random() * 10) + 1}/10, udah kayak rating ojol aja`,
        `Waduh ${name} jomok banget sih, kompetitor berat pak RT`,
        `${name} jomoknya kebangetan dah, bikin adem ayem lingkungan`,
        `Hmm... ${name} sepertinya ${Math.floor(Math.random() * 80) + 20}% jomok, sisanya 20% kepo`,
        ...
        `Jomok meter: ${name} = ${Math.floor(Math.random() * 100)}% loading... error! kebanyakan jomok`,
    ...
    static async getRandomMemberForSiapa(supabase, question) {
      try {
        ...
        if (!question.includes('?')) {
          ...
          return sarkasticResponses[Math.floor(Math.random() * sarkasticResponses.length)];
        }
        ...
        const memberResponses = [
          `Yang ${cleanQuestion.replace('?', '')} adalah *${randomMember.crew}*! 🎯 (Berdasarkan survei lapangan yang sangat akurat)`,
          `Menurut algoritma canggih saya, yang ${cleanQuestion.replace('?', '')} adalah *${randomMember.crew}*! 📊`,
          ...
          `Plot twist! Yang ${cleanQuestion.replace('?', '')} ternyata *${randomMember.crew}*! Unexpected banget kan? 😱`,
    ...
    static formatArmadaInfo(data) {
      ...
      switch(data.status) {
        case 'Belum Diverifikasi':
          statusInfo = 
    `⚠️ ARMADA INI BELUM DIVERIFIKASI! ⚠️
    ...
    *Note: Jangan lupa foto yang bagus ya, jangan yang blur kayak masa depan*`;
          break;
        case 'Terverifikasi':
          statusInfo = '✅ ARMADA TELAH DIVERIFIKASI DAN RESMI DIGUNAKAN! Congrats! 🎉';
          break;
        case 'Tidak Aktif':
      ...
      const armadaEmojis = ['🚌', '🚐', '🚍', '🚎'];
      const randomEmoji = armadaEmojis[Math.floor(Math.random() * armadaEmojis.length)];
